...
{
    public class WeatherStringQueryConversion
    {
        private static readonly string attributeName = "awx_weatherstring";
        ...
        {
            tracer.Trace(
                nameof(WeatherStringQueryConversion)
                    + "."
                    + nameof(GetFilteredCollection)
                    + $" Start: {DateTime.Now}"
            );

            var filteredMetars = new EntityCollection();
            if (exp.AttributeName != attributeName)
            {
                tracer.Trace(
                    $"Attribute name {exp.AttributeName} does not match the expected attribute name {attributeName}"
                );
